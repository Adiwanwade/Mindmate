...
const backendUrl = rawBackendUrl
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `process.env.BACKEND_URL/journal-delete/${username}/${journalToDelete}`
      );
      // Remove the deleted journal from the state
      setJournals(
        journals.filter((journal) => journal._id !== journalToDelete)
      );
        ...
        <div className="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 border-t border-gray-200 pt-10 sm:mt-16 sm:pt-16 lg:mx-0 lg:max-w-none lg:grid-cols-3">
          {journals.map((journal) => {
            const coverImage = journal.coverPicture
              ? `process.env.BACKEND_URL/${journal.coverPicture}`
              : defaultCoverImage;
